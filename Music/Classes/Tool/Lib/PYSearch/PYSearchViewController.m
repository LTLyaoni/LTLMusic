...
@property (nonatomic, weak) UINavigationController *searchSuggestionNav;
/** 热门标签容器 */
...
- (UIViewController *)searchResultController
{
    if (!_searchResultController) {
        _searchResultController = [[UIViewController alloc] init];
        self.searchResultTableView = _searchResultController.view;
    }
...
- (PYSearchSuggestionViewController *)searchSuggestionVC
{
    if (!_searchSuggestionVC) {
        ...
        searchSuggestionVC.tableView.contentInset = UIEdgeInsetsMake(44, 0, 0, 0);
...
- (void)setup
{
    ...
    UIImage *Image = [LTLMagnifier imageOfArtboardWithSize:CGSizeMake(30, 30) resizing:LTLMagnifierResizingBehaviorAspectFit];
    
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:Image style:UIBarButtonItemStyleDone target:self action:@selector(cancelDidClick)];
    ...
    // 创建搜索框
    UISearchBar *searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(0, 0, 400, 28)];
    ...
    // 开启一个通知接受,开始播放
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(exitKeyboard) name:LTLExitKeyboard object:nil];
...
- (NSArray *)addAndLayoutTagsWithTagsContentView:(UIView *)contentView tagTexts:(NSArray<NSString *> *)tagTexts;
{
    ...
    // 设置头部高度
    self.baseSearchTableView.tableHeaderView.py_height = self.headerContentView.py_height = self.headerView.py_height =CGRectGetMaxY(contentView.frame) + PYMargin * 2;
    self.baseSearchTableView.tableHeaderView.hidden = NO;
//    self.baseSearchTableView.tableHeaderView = self.headerView;
    // 重新赋值, 注意：当操作系统为iOS 9.x系列的tableHeaderView高度设置失效，需要重新设置tableHeaderView
    [self.baseSearchTableView setTableHeaderView:self.baseSearchTableView.tableHeaderView];
    return [tagsM copy];
}
...
//    self.searchSuggestionVC.searchSuggestions = [searchSuggestions copy];
     self.searchSuggestionVC.searchSuggestions = searchSuggestions;
}
    ...
    switch (searchHistoryStyle) {
        ...
        case PYSearchHistoryStyleBorderTag: // 边框标签
            for (UILabel *tag in self.searchHistoryTags) {
                ...
                // 设置边框颜色
                tag.layer.borderColor = self.borderColor.CGColor;
                // 设置边框宽度
...
{
    if (!_borderColor) {
        _borderColor = PYColor(223, 223, 223);
    }
    return _borderColor;
}
...
- (void)cancelDidClick
{
    ...
    [self searchBarSearch:self.searchBar];
}
...
{
    [self searchBarSearch:searchBar];
}
-(void)searchBarSearch:(UISearchBar *)searchBar
{
    // 回收键盘
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText
{
    ...
    // 滚动时，回收键盘
    if (!self.keyboardshowing) [self.searchBar becomeFirstResponder];
    
    // 根据输入文本显示建议搜索条件
    self.searchSuggestionVC.search = searchText;
